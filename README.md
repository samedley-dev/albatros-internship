...
Если в датасете есть хотя бы один дубликат или разрыв, то он считается невалидным. Фрагмент кода из файла `data_validation.py`, реализующий функцию `verify(df)` для установления валидности датасета:
...
В данных могут иметься статистические выбросы. Для их обработки была реализована функция `processed_df(filename)`, возвращающая пандас-датафрейм, содержащий данные о продажах, очищенные от выбросов. Ниже приведен код из файла `outlier_processing.py`, реализующий работу данной функции.
