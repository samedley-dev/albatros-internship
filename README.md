...
Использование модели Facebook Prophet ограничилась не только обучением модели и предсказанием значений целевой переменной, но и подбором параметров, при которых модель будет получать наиболее точные прогнозы. Для этого была реализована функция `objective(args)`, используя которую мы реализовали оптимизацию гиперпараметров:
...
Фрагмент из файла `prophet_forecast`, реализующий прогнозирование продаж модели Facebook Prophet с оптимизированными гиперпараметрами (при помощи байесовской оптимизации, реализованной методом `mango.Tuner().minimizer()`, приведен ниже.
